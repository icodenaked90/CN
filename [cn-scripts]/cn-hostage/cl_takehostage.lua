...
RegisterCommand("taggidsel",function()
	takeHostage()
end)

RegisterCommand("tg",function()
	takeHostage()
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Du skal bruge en pistol for at tage et gidsel.")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("Ingen person i nærheden.")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Tryk [G] for at slippe løs, [K] for at dræbe.")
			if IsDisabledControlJustPressed(0,47) then --release
				...
				Wait(100)
				releaseHostage()
			elseif IsDisabledControlJustPressed(0,311) then --kill 
