...
local prices = {
    [11] = {
        ...
        [1] = {price=35000,title="installeret <b style='color: #4E9350'>Chiptuning</b>"},
        [2] = {price=45000,title="installeret <b style='color: #4E9350'>Sport Motor</b>"},
        [3] = {price=55000,title="installeret <b style='color: #4E9350'>Racer Motor</b>"},
    },
    [12] = {
        [0] = {price=0,title="installeret <b style='color: #4E9350'>Standard Bremser</b>"},
        [1] = {price=22500,title="installeret <b style='color: #4E9350'>Gade Bremser</b>"},
        [2] = {price=35000,title="installeret <b style='color: #4E9350'>Sport Bremser</b>"},
        [3] = {price=49500,title="installeret <b style='color: #4E9350'>Racer Bremser</b>"},
    },
    [13] = {
        [0] = {price=0,title="installeret <b style='color: #4E9350'>Standard Gearkasse</b>"},
        [1] = {price=20000,title="installeret <b style='color: #4E9350'>Gade Gearkasse</b>"},
        [2] = {price=35000,title="installeret <b style='color: #4E9350'>Sport Gearkasse</b>"},
        [3] = {price=45000,title="installeret <b style='color: #4E9350'>Racer Gearkasse</b>"},
    },
    ["turbo"] = {
        ["off"] = {price=0,title="fjernet <b style='color: #DB4646'>Turbo</b>"},
        ["on"] = {price=50000,title="installeret <b style='color: #4E9350'>Turbo</b>"},
    }
}
...
AddEventHandler('cn-mech:upgradeveh', function(user_id,veh,mod,lvl,toggle)
    ...
    if item ~= nil and item.price >= 0 then
        ...
        if vRP.tryFullPayment({nuser_id,tonumber(item.price)}) then
            ...
            local command = ""
            if mod == 11 then
                command = "UPDATE vrp_user_vehicles SET vehicle_mods11 = @lvl WHERE user_id = @user_id AND vehicle = @veh"
            elseif mod == 12 then
                command = "UPDATE vrp_user_vehicles SET vehicle_mods12 = @lvl WHERE user_id = @user_id AND vehicle = @veh"
            elseif mod == 13 then
                command = "UPDATE vrp_user_vehicles SET vehicle_mods13 = @lvl WHERE user_id = @user_id AND vehicle = @veh"
            elseif mod == "turbo" then
                command = "UPDATE vrp_user_vehicles SET vehicle_turbo = @lvl WHERE user_id = @user_id AND vehicle = @veh"
            end
            if command ~= "" then
                MySQL.Async.execute(command, {user_id = user_id, veh = veh, lvl = lvl})
...
AddEventHandler('cn-mech:impoundveh', function(user_id,plate,veh,bool,netveh)
    plate = "P "..plate
    if bool == 0 then
        MySQL.Async.execute("UPDATE vrp_user_vehicles SET vehicle_impound = @impound,vehicle_impound_reason = @reason WHERE user_id = @user_id AND vehicle = @veh", {user_id = user_id,veh=veh,impound=bool,reason=""})
    ...
    elseif bool == 1 then
        ...
        vRP.prompt({source, firstToUpper(veh).." ("..plate..") beslaglagt årsag:", "", function(player,reason)
            if string.len(reason) >= 2 and string.len(reason) <= 200 then
                MySQL.Async.execute("UPDATE vrp_user_vehicles SET vehicle_impound = @impound,vehicle_impound_reason = @reason WHERE user_id = @user_id AND vehicle = @veh", {user_id = user_id,veh=veh,impound=bool,reason=reason})
...
function sendToDiscord(name, message)
    ...
    PerformHttpRequest("https://discordapp.com/api/webhooks/597826642819416066/FDEJgX-olbdej52Ra6a4m7cYlsR76Zi_ehItyZ1AHEiqSeKVEUvP4rLpZ2cg7JiOQCTo", function(err, text, headers) end, 'POST', json.encode({avatar_url = "https://i.imgur.com/HUgauWf.png",username = name,content = message}), { ['Content-Type'] = 'application/json' })
end
