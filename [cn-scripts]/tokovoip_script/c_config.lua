TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "MAIN",
		TSPassword = "branco4manager", -- TeamSpeak channel password (can be empty)
		...
		TSServer = "ts.fivem.dk", -- TeamSpeak server address to be displayed on blocking screen
		TSChannelSupport = "HJÆLP", -- TeamSpeak support channel name displayed on blocking screen
		TSDownload = "https://ts.fivem.dk", -- Download link displayed on blocking screen
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			...
			TokoVoipConfig.plugin_data.localName = escape("CiviliansNetwork"); -- Set the local name
		end);
