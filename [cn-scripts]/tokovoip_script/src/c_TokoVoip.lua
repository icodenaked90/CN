...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if (string.match(self.myChannels[self.plugin_data.radioChannel].name, "Dark Radio")) then
...
function TokoVoip.initialize(self)
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1) and health >= 101 then -- Talk on radio
				...
				if (lastTalkState == false and self.myChannels[self.plugin_data.radioChannel]) then
					if (not string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call33") and not IsPedSittingInAnyVehicle(PlayerPedId())) then
