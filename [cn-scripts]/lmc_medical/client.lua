...
AddEventHandler('medSystem:near', function(x,y,z, pulse, blood, name, area, bldn)
		...
		Wait(1)
			while timer >= 1 do
				...
				if cPulse ~= -1 and cBlood ~= -1 then
					DrawAdvancedText(0.7, 0.7, 0.005, 0.0028, 0.9, cName .. "\n~r~Puls: ~w~" .. cPulse .. " BPM~r~\nSåret: ~w~" .. cArea, 255, 255, 255, 255, 4, 1)
...
AddEventHandler('medSystem:send', function(player, source)
	print(source)
	...
	print(a)
	TriggerServerEvent('medSystem:print', player, math.floor(pulse * (blood / 90)), area, blood, a, b, c, bleeding)
