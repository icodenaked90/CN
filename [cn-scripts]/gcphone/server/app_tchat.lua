...
function TchatGetMessageChannel (channel, cbr)
  ...
  MySQL.Async.fetchAll("SELECT * FROM phone_app_chat WHERE channel = @channel ORDER BY time DESC LIMIT 100", {channel = channel}, function (rows)
    if #rows > 0 then
      task({rows})
    end
  end)
...
function TchatAddMessage (userid, channel, message)
  ...
  MySQL.Async.execute("INSERT INTO phone_app_chat (`userid`, `channel`, `message`) VALUES(@userid, @channel, @message)", {userid = userid, channel = channel, message = message}, function(rows)
  end)


  MySQL.Async.fetchAll("SELECT * from phone_app_chat WHERE `id` ORDER BY id DESC LIMIT 0, 1", {}, function (rows)
    if #rows > 0 then
      TriggerClientEvent('gcPhone:tchat_receive', -1, {channel=rows[1].channel,message=rows[1].message,time=rows[1].time,userid=rows[1].userid})
    end
  end)
