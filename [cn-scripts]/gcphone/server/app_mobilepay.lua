    ...
    if #result > 0 then
      ...
      if tosource ~= nil then
        if vRP.tryBankPayment({user_id,tonumber(data.money)}) then
          ...
          PerformHttpRequest('https://discordapp.com/api/webhooks/597825761478443017/P37vgxGGHZyRz1zTHKvvH1cTME_dO_xbVH7u57v9kM_EcV0gmK0wcVQN3UcRWwTTLu0S', function(err, text, headers) end, 'POST', json.encode({username = "Server "..GetConvar("servernumber", "0").." - Mobilepay", content = "**"..user_id.."** har lige sendt **"..format_thousand(tonumber(data.money)).."** kr. (Med gebyr: "..format_thousand(tonumber(moneyToUser))..") til **"..touserid.."** (**"..os.date("%H:%M:%S %d/%m/%Y").."**)"}), { ['Content-Type'] = 'application/json' })
