...
-- set mysql_connection_string "server=localhost;uid=root;password=;database=vrp;sslmode=none"
...
MySQL.ready(function ()
  MySQL.Async.execute("DELETE FROM phone_messages WHERE (DATEDIFF(CURRENT_DATE,time) > 20)", nil, function(rowsChanged)
    print("Slettede " .. rowsChanged .. " beskeder")
  end)
  MySQL.Async.execute("DELETE FROM phone_calls WHERE (DATEDIFF(CURRENT_DATE,time) > 20)", nil, function(rowsChanged)
    print("Slettede " .. rowsChanged .. " opkald")
  end)
  MySQL.Async.execute("DELETE FROM phone_messages WHERE (`transmitter` like '%-Job' or `transmitter` = 'Taxi') AND (DATEDIFF(CURRENT_DATE,time) > 1)", nil, function(rowsChanged)
    print("Slettede " .. rowsChanged .. " service beskeder")
  end)
end)
...
function _internalAddMessage(transmitter, receiver, message, owner, cbr)
    ...
    MySQL.Async.execute("INSERT INTO phone_messages (transmitter, receiver, message, isRead,owner) VALUES(@transmitter, @receiver, @message, @isRead, @owner)", {transmitter = transmitter,receiver = receiver, message = message, isRead = owner, owner = owner}, function(rows)
    end)

    -- local lastInsertId = MySQL.Async.fetchAll("SELECT LAST_INSERT_ID()", {}, function (rows) end)
    -- print(lastInsertId .. "test")
    MySQL.Async.fetchAll("SELECT * from phone_messages ORDER BY id DESC LIMIT 0, 1", {lastInsertId = lastInsertId}, function (rows)
      if #rows > 0 then
          task({rows[1]})
      end
    end)
    ...
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            ...
            if num ~= nil and mePhoneNumber ~= nil then
              MySQL.Async.execute("UPDATE phone_messages SET phone_messages.isRead = 1 WHERE phone_messages.receiver = @receiver AND phone_messages.transmitter = @transmitter", {receiver = mePhoneNumber, transmitter = num}, function()
              end)
...
function deleteMessage(msgId)
    ...
    MySQL.Async.execute("DELETE FROM phone_messages WHERE `id` = @id", {id = msgId}, function()
    end)
end
...
function deleteAllMessageFromPhoneNumber(identifier, phone)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            ...
            MySQL.Async.execute("DELETE FROM phone_messages WHERE receiver = @mePhoneNumber and transmitter = @phone", {mePhoneNumber = mePhoneNumber, phone = phone}, function()
            end)
...
function deleteAllMessage(identifier)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            ...
            MySQL.Async.execute("DELETE FROM phone_messages WHERE receiver = @mePhoneNumber", {}, function()
            end)
        end
    end})
end
...
function getHistoriqueCall (num, cbr)
    ...
    MySQL.Async.fetchAll("SELECT * FROM phone_calls WHERE phone_calls.owner = @num ORDER BY time DESC LIMIT 25", {num = num}, function (rows)
      if #rows > 0 then
        task({rows})
      end
    end)
...
function saveAppels (appelInfo)
    if appelInfo.extraData == nil or appelInfo.extraData.useNumber == nil then
        ...
        MySQL.Async.execute("INSERT INTO phone_calls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)", {owner = appelInfo.transmitter_num, num = appelInfo.receiver_num, incoming = 1, accepts = appelInfo.is_accepts}, function()
          notifyNewAppelsHisto(appelInfo.transmitter_src, appelInfo.transmitter_num)
        end)
    ...
    if appelInfo.is_valid == true then
        ...
        MySQL.Async.execute("INSERT INTO phone_calls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)", {owner = appelInfo.receiver_num, num = num, incoming = 0, accepts = appelInfo.is_accepts}, function()
            if appelInfo.receiver_src ~= nil then
                notifyNewAppelsHisto(appelInfo.receiver_src, appelInfo.receiver_num)
            end
        end)
...
AddEventHandler('gcPhone:appelsDeleteHistorique', function (numero)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            ...
            MySQL.Async.execute("DELETE FROM phone_calls WHERE `owner` = @owner AND `num` = @num", {owner = mePhoneNumber, num = numero}, function()
            end)
        end
    end})
...
function appelsDeleteAllHistorique(srcIdentifier)
    vRP.getUserIdentity({srcIdentifier, function(identity)
        if identity then
            ...
            MySQL.Async.execute("DELETE FROM phone_calls WHERE `owner` = @owner", {owner = mePhoneNumber}, function()
            end)
...
AddEventHandler('gcPhone:allUpdate', function(user_id, source)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            ...
            MySQL.Async.fetchAll("SELECT * FROM phone_messages WHERE receiver = @phone", {phone = phone}, function (rows)
              if #rows > 0 then
                  TriggerClientEvent("gcPhone:allMessage", lsource, rows)
...
RegisterCommand('gcphone', function(source)
  local source = source
  local user_id = vRP.getUserId({source})
  TriggerEvent("gcPhone:allUpdate", user_id,source)
  TriggerClientEvent("pNotify:SendNotification", source,{text = "Du genopfriskede din GCPhone data", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
...
AddEventHandler('onResourceStart', function(name)
    if name:lower() == GetCurrentResourceName():lower() then
        ...
        MySQL.Async.fetchAll("SELECT name,message,time FROM `phone_twitter` ORDER by id DESC LIMIT 50", {}, function (rows)
          if #rows > 0 then
            tweets = json.encode(rows)
          end
        end)
