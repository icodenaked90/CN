    ...
    while true do
        ...
        if menuIsOpen == true then
            for _, value in ipairs(KeyToucheCloseEvent) do
              --  TriggerEvent("els:blokerknap", true)
                if IsControlJustPressed(1, value.code) then
                    SendNUIMessage({keyUp = value.event})
                end
            end
          else
          --  TriggerEvent("els:blokerknap", false)
        end
    end
end)
...
RegisterNUICallback('reponseText', function(data, cb)
    ...
    DisplayOnscreenKeyboard(1, "BESKED", "", text, "", "", "", limit)
    while (UpdateOnscreenKeyboard() == 0) do
        DisableAllControlActions(0);
        Wait(0);
    end
...
-- AddEventHandler('onResourceStart', function(source)
--   local user_id = vRP.getUserId({source})
--   TriggerServerEvent("gcPhone:allUpdate", user_id,source)
-- end)
