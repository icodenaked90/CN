...
local allowedTowModels = {
    ...
    ['DA_MTLFB2013M'] = {x = 0.0, y = -1.2, z = 0.95}, -- addon Domingo Auto
    ['DA_FF350RT2017M'] = {x = 0.0, y = -2.22, z = 1.00}, -- addon Domingo Auto
    ['SM_MTLFB2013M'] = {x = 0.0, y = -1.2, z = 0.95}, -- addon Super Mechanics
}
local allowedBensonModels = {
    ['benson'] = {x = 0.0, y = -0.0, z = 0.0}, -- addon benson
    ['TITAN'] = {x = 0.0, y = -0.0, z = 0.0}, -- addon benson
}
...
function isVehicleABenson(vehicle)
    local isValid = false
    for model,posOffset in pairs(allowedBensonModels) do
        if IsVehicleModel(vehicle, model) then
            xoff = posOffset.x
            yoff = posOffset.y
            zoff = posOffset.z
	...
	if isVehicleTow then
		...
		Citizen.CreateThread(function()
			while true do
				...
				if (IsEntityUpsidedown(GetVehiclePedIsIn(PlayerPedId(), true)) or roll > 70.0 or roll < -70.0) and isVehicleTow then
...
RegisterNetEvent('hide')
AddEventHandler('hide', function()
        local playerid = PlayerPedId()
        local targetVehicle = GetVehiclePedIsIn(playerid, true)

        local coordA = GetEntityCoords(playerid, 1)
        local coordB = GetOffsetFromEntityInWorldCoords(playerid, 0.0, 5.0, 0.0)
        local vehicle = getVehicleInDirection(coordA, coordB)
        if currentlyTowedVehicle == nil then
            if not ((not allowTowingBoats and IsThisModelABoat(targetModelHash)) or (not allowTowingHelicopters and IsThisModelAHeli(targetModelHash)) or (not allowTowingPlanes and IsThisModelAPlane(targetModelHash)) or (not allowTowingTrains and IsThisModelATrain(targetModelHash)) or (not allowTowingTrailers and isTargetVehicleATrailer(targetModelHash))) then
                if vehicle ~= targetVehicle and isVehicleABenson(vehicle) then
                    AttachEntityToEntity(targetVehicle, vehicle, GetEntityBoneIndexByName(vehicle, 'bodyshell'), 0.0 + xoff, -1.5 + yoff, 0.5 + zoff, 0, 0, 0, 1, 1, 0, 1, 0, 1)
                    currentlyTowedVehicle = targetVehicle
                    TriggerEvent("pNotify:SendNotification",{text = "Køretøjet blev kørt på ladet",type = "success",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                else
                    TriggerEvent("pNotify:SendNotification",{text = "Dette er den forkerte Lastbil",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                end
            end
        else isVehicleABenson(vehicle)
            DetachEntity(currentlyTowedVehicle, false, false)
            local vehiclesCoords = GetOffsetFromEntityInWorldCoords(vehicle, 0.0, -10.0, 0.0)
            SetEntityCoords(currentlyTowedVehicle, vehiclesCoords["x"], vehiclesCoords["y"], vehiclesCoords["z"], 1, 0, 0, 1)
            SetVehicleOnGroundProperly(currentlyTowedVehicle)
