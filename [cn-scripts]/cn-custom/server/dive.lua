...
AddEventHandler('dive:gear', function(swim)
	...
	if vRP.tryGetInventoryItem({user_id,"dive",1}) == true then
		...
		Citizen.Wait(10000)
		TriggerClientEvent("pNotify:SendNotification",player,{text = "Forlader du vandet smider du dykker udstyret.",type = "success",timeout = (2000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
	else
		--nothing
	end
end)
