...
local function blackout()
	...
	if not isBlackedOut then
		...
		Citizen.CreateThread(function()
			...
			TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 50.0, 'crash01', 2.5) -- Trigger crash sound around yourself, works with InteractiveSound
			if impact <= 50 then -- Shakycam on impact
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 1.4)
			...
			elseif impact > 70 and impact <= 80 then
				...
				Citizen.Wait(30000)
				SetEntityHealth(GetPlayerPed(-1), 105) -- This setting cause death to passengers, try 100 if you want very low life.
			end
