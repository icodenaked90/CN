...
AddEventHandler("playerConnecting", function(name, reject, def)
    local serverstatus = vRP.getServerStatus()
    if serverstatus == false then
        ...
        if not Rocade(steamID, def, source,name) then
            CancelEvent()
        end
    else
        def.defer()
        def.update("Serveren er igang med at "..serverstatus)
    end
end)

-- Fonction principale, utilise l'objet "deferrals" transmis par l'evenement "playerConnecting"
function Rocade(steamID, def, source,name)
    ...
    def.update("Tjekker om du er banned...")

    local ids = GetPlayerIdentifiers(source)
    if ids ~= nil and #ids > 0 then
        def.update("Tjekker identifikatorer...")
        vRP.getUserIdByIdentifiers({ids, function(user_id)
            vRP.getUserData({user_id, function(userdata)
                if not userdata.banned then
                    def.update("Tjekker om du er whitelisted...")
                    if userdata.whitelisted then
                        ...
                        vRP.startLoadingIn({source,user_id,steamID,name,def})

                        return true
                    else
                        print(name.." ("..getPlayerEnd(source)..") afvist: ikke whitelisted ("..user_id..")")
                        def.done("Ikke whitelisted ansøg på Discord.gg/AxpEspz (ID: "..user_id..")")
                        return true
                    end
                else
                    vRP.setDiscord({source,user_id,userdata.discord})
                    ...
                    local ep = getPlayerEnd(source)
                    if ep ~= userdata.last_login and ep ~= "0.0.0.0" then
                        vRP.updateIP({user_id,ep})
                    end

                    print(name.." ("..getPlayerEnd(source)..") afvist: banned ("..user_id..")")
                    def.done("Du er bannet. (ID: "..user_id..")")
                    return true
                end
        ...
        print(name.." ("..getPlayerEnd(source)..") afvist: mangler steamid")
        def.done("Tjek om du har dit steam åben, og ellers prøv at genstarte det.")
    end
end

function getPlayerEnd(source)
    return GetPlayerEndpoint(source) or "0.0.0.0"
end

local changedtitle = false
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local title = ""
        if #GetPlayers() > 26 and #waiting > 1 then
            local color = "^2"
            if #waiting >= 3 and #waiting < 6 then
                color = "^3"
            elseif #waiting > 5 then
                color = "^1"
            end
            title = color.."[KØ: "..tostring(#waiting).."] "..GetConvar("serverhostname", "^3[DK] ^5CiviliansNetwork ^3Server #%number ^1» ^3Whitelist 16+ ^1» ^3Discord.gg/AxpEspz")
            title = title:gsub("%%number", GetConvar("servernumber", 0))
            changedtitle = true
            ExecuteCommand('sv_hostname "'..title..'"')
        elseif changedtitle then
            title = GetConvar("serverhostname", "^3[DK] ^5CiviliansNetwork ^3Server #%number ^1» ^3Whitelist 16+ ^1» ^3Discord.gg/AxpEspz")
            title = title:gsub("%%number", GetConvar("servernumber", 0))
            ExecuteCommand('sv_hostname "'..title..'"')
        end
        CheckConnecting()
