...
AddEventHandler('iens:notAllowed', function()
	TriggerEvent("pNotify:SendNotification",{
		text = "Du har ikke tidlladelse til dette!",
		type = "warning",
		timeout = (3000),
		layout = "bottomCenter",
		queue = "global",
	...
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			if healthEngineCurrent <= 200 then
				...
				if speed > 80 then
					SetVehicleForwardSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 22.0)
				end
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			if healthEngineNew ~= healthEngineCurrent or refreshHealth ~= false then
				if refreshHealth ~= false then
					healthEngineNew = refreshHealth
					refreshHealth = false
				end
...
RegisterNetEvent("cn-damage:changehealth")
AddEventHandler("cn-damage:changehealth", function(dmg)
	refreshHealth = dmg
end)