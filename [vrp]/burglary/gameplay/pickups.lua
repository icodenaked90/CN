		...
		if isHolding then
			...
			if IsControlJustPressed(0, 38) then	
				...
				if isInHouse then
					...
					if Vdist(markerCoords, coords) < 3 then
						...
						-- send to server
						TriggerServerEvent("burglary:collected", currentItem.id, currentItem.house,source)
						...
						ClearPedTasks(PlayerPedId())	
						DetachEntity(holdingProp)
						TriggerServerEvent("burglary:getstolengoods")
						Wait(1000)
			...
			for _,pickup in pairs(pickups) do
				...
				if onMission and Vdist(coords, GetEntityCoords(pickup.prop)) < 2 and not isHolding then
					UI.DrawText3D(pickupCoord.x, pickupCoord.y, pickupCoord.z, "[~g~E~s~] for at tage denne ting!")
