...
local priceperdamage = 1000

local lsc = {
	inside = false,
	title = "CiviliansNetwork Mekaniker",
	currentmenu = "repair",
	lastmenu = nil,
	currentpos = nil,
	currentgarage = 0,
	...
	locations = { -- LSCUSTOMS LOCATIONS -- ADD SERVER LOCKS TOO
		[1] = {ishidden = true, locked = false, outside = { x = 614.30395507813, y = 2516.171875, z = -154.154296875, heading = 71.187133}, inside = {x = -337.3863,y = -136.9247,z = 38.5737, heading = 269.455}},
		...
		[7] = { ishidden = true, locked = false, outside = { x = 464.025390625,y=-1016.9728393555,z=28.083700180054, heading = 0.450}, inside = {x = 464.025390625,y=-1016.9728393555,z=28.083700180054, heading = 182.402}},
		[8] = { ishidden = true, locked = false, outside = { x = -1758.5438232422, y= 3050.6447753906, z= 32.211769104004, heading = 0.450}, inside = {x = -1758.5438232422, y= 3050.6447753906, z= 32.211769104004, heading = 182.402}}, --Drugdealer
		[9] = { ishidden = true, locked = false, outside = { x = 883.28497314453,y= -3240.7124023438, z=-98.276245117188, heading = 0.450}, inside = {x = 883.28497314453, y= -3240.7124023438,z = -98.276245117188, heading = 182.402}},
		[10] = { locked = false, outside = { x = -69.222984313965,y= -1823.2701416016, z=26.94207572937, heading = 0.450}, inside = {x = -87.812599182129, y= -1811.1625976563,z = 26.958627700806, heading = 182.402}},
		[11] = { ishidden = true, locked = false, outside = { x = -194.16319274902, y= -287.17129516602, z= 48.970043182373, heading =69.450}, inside = {x = -194.16319274902, y= -287.17129516602, z= 48.970043182373, heading = 69.450}}, --AKS
	},
	menu = {
		x = 0.8,
		y = 0.1,
	...
	,["primarymetallic"] = {
		...
		["secondaryclassic"] = {
			...
			buttons = {
				...
				{name = "Sænket Affjedring",mod = false,modtype =15, costs = 150000, description = "", centre = 0, font = 0, scale = 0.4},
				{name = "Gade Affjedring",mod = 1,modtype =15, costs = 20000, description = "", centre = 0, font = 0, scale = 0.4},
				{name = "Sport Affjedring",mod = 2,modtype =15, costs = 25000, description = "", centre = 0, font = 0, scale = 0.4},
				{name = "Konkurrence Affjedring",mod = 3,modtype =15, costs = 30000, description = "", centre = 0, font = 0, scale = 0.4}
			}
		},
		--[[["transmission"] = {
			title = "Gearkasse",
...
function DriveInGarage()
		...
		if DoesEntityExist(veh) then
			if damage < 995 or not AreAllVehicleWindowsIntact(veh) or IsVehicleDoorDamaged(veh,0) or IsVehicleDoorDamaged(veh,1) or IsVehicleDoorDamaged(veh,2) or IsVehicleDoorDamaged(veh,3) then
				if vRPex.getAmountOnline({"mechanic"}) > 0 then
					lsc.currentmenu = "main"
					TriggerEvent("pNotify:SendNotification",{text = "Du skal kontakte en mekaniker!",type = "info",timeout = (3000),layout = "bottomCenter",queue = "lsrepair0",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				else
					if GetVehicleClass(veh) == 13 then
						...
						if repairprice > 60000 then repairprice = 60000 end
			...
			for i = 0,16 do
				if GetNumVehicleMods(veh,i) ~= nil and GetNumVehicleMods(veh,i) ~= false then
					...
					elseif i == 13 then
						insrt(lsc.menu["main"].buttons, {name = "Gearkasse - Find Mekaniker", description = "", centre = 0, font = 0, scale = 0.4})
					elseif i == 14 then
						insrt(lsc.menu["main"].buttons, {name = "Horn", description = "", centre = 0, font = 0, scale = 0.4})
					elseif i == 12 then
						insrt(lsc.menu["main"].buttons, {name = "Bremser - Find Mekaniker", description = "", centre = 0, font = 0, scale = 0.4})
					elseif i == 11 then
						insrt(lsc.menu["main"].buttons, {name = "Motor - Find Mekaniker", description = "", centre = 0, font = 0, scale = 0.4})
					elseif i == 0 then
			...
			insrt(lsc.menu["main"].buttons, {name = "Turbo - Find Mekaniker", description = "", centre = 0, font = 0, scale = 0.4})
	...
	while true do
		...
		if lsc ~= nil and lsc.inside == false then
			...
			if IsPedSittingInAnyVehicle(ped) then
				local veh = GetVehiclePedIsUsing(ped)
				if DoesEntityExist(veh) and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)) or IsThisModelAQuadbike(GetEntityModel(veh))) then
					for i,pos in ipairs(lsc.locations) do
						...
						if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
							if pos.locked == false then
								DrawText3d(outside.x,outside.y,outside.z+0.2, "Tryk ~g~E~w~ for at gå til mekanikeren")
								...
								DrawText3d(outside.x,outside.y,outside.z+0.2, "~r~Optaget, vent")
							end
						end
					end
				end
...
function ButtonSelected(button)
	...
	local repairprice = pricecalc * priceperdamage
	if repairprice > 60000 then repairprice = 60000 end
	if repairprice == 0 then repairprice = 2500 end
	if lsc.currentmenu == "repair" then
		if button.name == "Reparer køretøj" and isRepairing == false then
			TriggerEvent("pNotify:SendNotification",{text = "<center><h3>🛠️ Du påbegyndte reparation af dit køretøj</h3> <br> <b style='color: #FFC832'>"..math.floor(pricecalc).."</b> sekunder før mekanikeren er færdig! <br> forlad <b style='color: #CD5C5C'>IKKE</b> mekanikeren!",type = "info",timeout = (5000),layout = "centerRight",queue = "error",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
			isRepairing = true
			Citizen.CreateThread(function()
				local time = pricecalc * ms
				while true do
					if inCustoms == false then
						isRepairing = false
						TriggerEvent("pNotify:SendNotification",{text = "<center><h3>🛠️ Du kørte før mekanikeren blev færdig!</h3></center>",type = "info",timeout = (5000),layout = "centerRight",queue = "error",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
						return
					end
					if time > 10 then
						Citizen.Wait(1)
						time = time-10
						ply_drawTxt("Reparer dit køretøj: ~g~"..math.floor(time/ms).."~w~ sekunder",4,1,0.5,0.8,0.6,255,255,255,255)
					else
						Citizen.Wait(time)
