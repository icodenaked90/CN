    ...
    for l,w in pairs(users) do
      ...
      if not vRP.hasMission({player}) then
        if math.random(1,v.chance+1) == 1 then -- chance check
          -- build mission
          local mdata = {}
          mdata.name = lang.pilotstart({v.title})
          mdata.steps = {}

          -- build steps
          for i=1,v.steps do
            local step = {
              text = lang.pilotstart({v.title}).."<br />"..lang.reward({v.reward}),
              onenter = function(player, area)
                Mclient.AirplaneModelCheck(player, {}, function(airplane)
                  if car then
                    Mclient.freezePedVehicle(player,{true})
                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"Aflevere pakker"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                    SetTimeout(5000, function()
                      ...
                      if i == v.steps then
                        ...
                        vRPclient.notify(player,"Flyv til næste drstination.")
                      end
                    end)
                  else TriggerClientEvent("pNotify:SendNotification", player,{text = {"Du skal være i det rigtige fly"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                  end
                end)
              end,
            ...
            local step = {
              ...
              onenter = function(player, area)
                Mclient.CarModelCheck(player, {}, function(car)
                  if car then
                    Mclient.freezePedVehicle(player,{true})
                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"Samler Skrald"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                    SetTimeout(5000, function()
                      vRP.nextMissionStep({player})
                      Mclient.freezePedVehicle(player,{false})
                      -- last step
                  ...
                  else TriggerClientEvent("pNotify:SendNotification", player,{text = {"Du køre i det forkerte køretøj"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                  end
                end)
              end,
              position = v.positions[math.random(1,#v.positions+1)]
