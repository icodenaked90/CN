...
Citizen.CreateThread(function()
	...
	while true do
						...
						while done == false do
							...
							if(amountToEssence-0.0005 > 0) then
								...
								if(_essence > 0.142) then
									essence = 0.142
									TriggerEvent("advancedFuel:setEssence", 25, GetVehicleNumberPlateText(veh), GetDisplayNameFromVehicleModel(GetEntityModel(veh)))
									done = true
								end
		...
		if((isNearFuelStation or isNearFuelPStation or isNearFuelHStation or isNearFuelBStation) and not IsPedInAnyVehicle(GetPlayerPed(-1))) then
			drawTxt('[~g~E~s~] for at købe en dunk (~b~350 DKK~w~)', 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
	...
	local isEngineOn = Citizen.InvokeNative(0xAE31E7DF9B5B132E, GetVehiclePedIsIn(GetPlayerPed(-1)))
		...
		if isEngineOn then
			if isRadarExtended then
				...
				DrawAdvancedText(0.1550, 0.9855, 0.125, 0.0028, 0.45,round(percent,1), 255, 255, 255, 220, 0, 1)
			elseif(hud_form == 1) then
				local percent = (essence/0.145)*100
				DrawAdvancedText(0.1650, 0.9855, 0.125, 0.0028, 0.45,round(percent,1), 255, 255, 255, 220, 0, 1)
			else
				local percent = (essence/0.145)*100
			end
			DrawAdvancedText(0.1335, 0.9855, 0.125, 0.0028, 0.45,"~b~BENZIN: ", 255, 255, 255, 220, 0, 1)
			DrawAdvancedText(0.1825, 0.9855, 0.125, 0.0028, 0.45,"% ~b~ |", 255, 255, 255, 220, 0, 1)
		end
    end
end
