...
AddEventHandler("essence:buy", function(amount, index,e)
	...
	local toPay = round(amount*price)
	local user_id = vRP.getUserId({_source})
	if(vRP.tryFullPayment({user_id,toPay})) then
		TriggerClientEvent("essence:hasBuying", _source, amount, toPay)
	else
		TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har ikke penge nok.", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
...
AddEventHandler("essence:buyCan", function()
	...
	if(vRP.tryFullPayment({user_id,toPay})) then
		...
		TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har købt en benzindunk for <b style='color: #DB4646'>"..toPay.." DKK</b>.", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	else
		TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har ikke penge nok.", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
end)
...
function renderPrice()
    for i=0,35 do
        if(randomPrice) then
            StationsPrice[i] = math.random(10,15)
        else
