...
function Menu.updateImpounded(impound)
	Menu.Impound = impound
end
...
function Menu.renderButtons()
	...
	local thisoffset = 0.0125
	local thisheight = 0.049
	local thishoffset = 0.05
	if Menu.Impound > 0 then
		thishoffset = 0.07
		thisoffset = 0.03
		thisheight = 0.059
	end
	DrawText((xoffset + 0.07), (yoffset - 0.065 - thisoffset))
	if Menu.Impound > 0 then
		SetTextFont(4)
		SetTextProportional(0)
		...
		SetTextCentre(2)
		SetTextEntry("STRING")
		AddTextComponentString(Menu.Impound.."x Beslaglagte køretøjer")
		DrawText((xoffset + 0.07), (yoffset - 0.055))
	end

	Menu.renderBox(xoffset,0.3,(yoffset - thisheight),thishoffset,20,20,255,150)
