...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(1)
    if IsControlJustPressed(0,182) and GetLastInputMethod(0) then
...
function tvRP.spawnGarageVehicle(vtype,name,pos) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
  ...
  if vehicle == nil then
    ...
    if HasModelLoaded(mhash) then
      ...
      local x2,y2,z2 = tvRP.getPosition({})
...
function tvRP.fixeNearestVehicleMidlertidigt(radius)
  ...
  if IsEntityAVehicle(veh) then
    if not IsPedSittingInAnyVehicle(ped) then
      ...
      TriggerEvent("pNotify:SendNotification",{text = "Nærmeste køretøj er midlertidigt repareret.",type = "success",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
    else
      TriggerEvent("pNotify:SendNotification",{text = "Du må ikke sidde i køretøjet, mens du reparerer det.",type = "error",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
    end
  else
...
function tvRP.vehicleUnlockMekaniker()
  ...
  TriggerEvent("pNotify:SendNotification",{text = "Vent seks sekunder før køretøjet er låst op!",type = "error",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
  Citizen.Wait(6000)
  TriggerEvent("pNotify:SendNotification",{text = "Nærmeste køretøj låst op.",type = "error",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
end

function tvRP.vaskNearestBil()
  local veh = tvRP.getNearestVehicle(4)

  SetVehicleDirtLevel(veh, 0.0) --Vasker køretøjet
end
...
function tvRP.vc_breakOpen(vtype, door_index)
  local vehicle = vehicles[vtype]
  if vehicle then
    SetVehicleDoorBroken(vehicle[3],door_index,false)
  end
end
...
function tvRP.vc_toggleLock(vtype)
  ...
  if vehicle then
    ...
    TriggerEvent("cn-animation:startWithItem", "carkeys")
    ...
    if locked then -- unlock
      ...
      TriggerEvent("pNotify:SendNotification",{text = "Køretøj <b style='color:#DB4646'>låst</b>",type = "success",timeout = (3000),layout = "bottomCenter",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
    end
