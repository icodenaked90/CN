...
function ProgressBar(data)
{
  ...
  this.div.style.backgroundColor = "rgba("+data.r+","+data.g+","+data.b+",0.65)";
...
ProgressBar.prototype.frame = function(time)
{
  ...
  if(anchor){
    ...
    if(anchor_index >= 0){
      if(anchor_name == "minimap"){ //MINIMAP
      ...
      else if(anchor_name == "health"){ //HEALTH
        var width = cfg.anchor_health_width/anchor.length; //divide horizontal map space by number of pbars

        //set size
        this.div.style.width = this.div_label.style.width = (width)+"px";
        this.div_inner.style.height = this.div.style.height = this.div_label.style.height = (12)+"px";
        this.div_label.style.lineHeight = this.div_label.style.height;
        ...
        //set position
        this.div.style.left = (cfg.anchor_health_left+anchor_index*width)+"px";
        this.div.style.top = (document.body.offsetHeight-cfg.anchor_health_bottom)+"px";
      }
