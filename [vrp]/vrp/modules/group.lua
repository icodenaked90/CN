...
function vRP.getUserGroupRankup(gtype,group)
  local order = group ~= "" and groups[group]._config.order+1 or 1
  for k,v in pairs(groups) do
    local kgroup = groups[k]
    if kgroup then
      if kgroup._config and kgroup._config.gtype and kgroup._config.gtype == gtype and order == kgroup._config.order then
        return {k,kgroup._config.order}
      end
    end
  end
  return false
