...
function vRP.setMoney(user_id,value)
    ...
    if tmp then
        ...
        if twallet < newwallet then
            if newwallet-twallet > 2000000 then
                local banned = "Nej"
                if newwallet-twallet > 15000000 then
                    -- banned = "Ja"
                ...
                PerformHttpRequest("https://discordapp.com/api/webhooks/597824185875562496/mbYJUlwZ-g5BYSuFxYFY6nnQVwXH3zzJhAKie70cD-BJ_EZd9r6MkqmI6KWw-93lBL4Z", function(err, text, headers) end, 'POST', json.encode(
                    {
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
    ...
    if tmp and tmp.wallet ~= nil and tmp.bank ~= nil and tmp.debt ~= nil then
        MySQL.Async.execute("UPDATE vrp_user_moneys SET wallet = @wallet, bank = @bank, debt = @debt WHERE user_id = @user_id", {user_id = user_id, wallet = tmp.wallet, bank = tmp.bank, debt = tmp.debt})
    end
end)
...
AddEventHandler("vRP:save", function()
    for k,v in pairs(vRP.user_tmp_tables) do
        if v.wallet ~= nil and v.bank ~= nil then
            MySQL.Async.execute("UPDATE vrp_user_moneys SET wallet = @wallet, bank = @bank, debt = @debt WHERE user_id = @user_id", {user_id = k, wallet = v.wallet, bank = v.bank,debt=v.debt})
        end
    end
end)
                ...
                if nuser_id ~= nil then
                    ...
                    vRP.prompt(player,lang.money.give.prompt(),"",function(player,amount)
                        ...
                        if amount > 0 and vRP.tryPayment(user_id,amount) then
                            ...
                            TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.given({amount})}, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                            TriggerClientEvent("pNotify:SendNotification", nplayer,{text = {lang.money.received({amount})}, type = "success", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        else
