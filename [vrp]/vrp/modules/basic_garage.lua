        ...
        if user_id ~= nil then
            ...
            MySQL.Async.fetchAll("SELECT vehicle FROM vrp_user_vehicles WHERE user_id = @user_id", {user_id = user_id}, function(_pvehicles, affected)
                if #_pvehicles > 0 then
                    ...
                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"Du har ingen køretøjer!"}, type = "error", queue = "global", timeout = 2000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                end
            end)
        end
                            ...
                            vRP.prompt(player,"Pris i DKK: ","",function(player,amount)
                                ...
                                if amount then
                                    if amount >= 0 then
                                                        ...
                                                        vRP.request(target,"<b>"..fornavnP.." "..efternavnP.."</b> ønsker at sælge sin <b>"..name.."</b> til en pris på <b style='color: #96D3FF'>"..amount.." DKK</b>", 10, function(target,ok)
                                                            if ok then
                                                                ...
                                                                if vRP.tryFullPayment(pID, amount) then
                                                                    ...
                                                                    PerformHttpRequest('https://discordapp.com/api/webhooks/597828312223711232/4IpbxjT8Jy-8UR6vKyYzaBfTBax1DsCH_f7sKaUmFdniJOctyBVO_M-naLVE197Ns4q7', function(err, text, headers) end, 'POST', json.encode({username = "Salg af køretøj - Server " .. GetConvar("servernumber","0"), content = "**"..fornavnP.." "..efternavnP.." ("..p_id..")** solgte sit køretøj **"..name.."** til **"..fornavnT.." "..efternavnT.." ("..t_id..")** for **"..amount.." DKK**."}), { ['Content-Type'] = 'application/json' })
                                                                else
                                                                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"<b>"..fornavnT.." "..efternavnT.."</b> har ikke råd."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                                                    TriggerClientEvent("pNotify:SendNotification", target,{text = {"Du har ikke nok penge på dig."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                                                end
                                                            else
                                        ...
                                        TriggerClientEvent("pNotify:SendNotification", player,{text = {"Prisen skal være højere eller lig med 0 DKK."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                    end
                                else
                                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"Prisen skal være et tal."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                end
                            end)
                        else
                            TriggerClientEvent("pNotify:SendNotification", player,{text = {"Dette ID ser ud til ikke at eksistere."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        end
                    else
                        TriggerClientEvent("pNotify:SendNotification", player,{text = {"Intet ID valgt."}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                    end
                end)
            else
...
local function ch_repair(player,choice)
    ...
    if user_id ~= nil then
        -- anim and repair
        TriggerClientEvent("cn-animation:startWithItem", player, "repair")
        SetTimeout(26000, function()
...
-- Vask det nærmeste køretøj.
local function ch_vaskBil(player, choice)
    vRPclient.vaskNearestBil(player)
end

local function ch_vaskBil(player,choice)
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
      TriggerClientEvent("pNotify:SendNotification", player, {text = "Begynder at vaske, vent omkring 1 minut!",type = "error",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        TriggerClientEvent("cn-animation:startWithoutNotify", player, "vask")
        Wait(60000)
        vRPclient.vaskNearestBil(player)
        TriggerClientEvent("cn-animation:stopAnim", player)
        TriggerClientEvent("pNotify:SendNotification", player, {text = "Køretøjet blev vasket!",type = "success",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      else
        TriggerClientEvent("pNotify:SendNotification", player, {text = "Intet køretøj i nærheden!",type = "warning",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
end
                ...
                if nuser_id ~= nil and nuser_id ~= "" then
                    ...
                    if target ~= nil then
                        ...
                        if mechlvl < 3 then
                            ...
                            TriggerClientEvent("pNotify:SendNotification", player,{text = {"Du har givet et certifikat til <b style='color: #DB4646'>"..nuser_id.."</b>"}, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                            local user_id = vRP.getUserId(player)
                            local dato = os.date("**%d-%m-%Y** kl. **%X**")
                            local dmessage = "**"..user_id.."** har lige givet et certifikat til **"..nuser_id.."** (**LVL: "..(mechlvl+1).."**) ("..dato..")"
                            PerformHttpRequest('https://discordapp.com/api/webhooks/597828312223711232/4IpbxjT8Jy-8UR6vKyYzaBfTBax1DsCH_f7sKaUmFdniJOctyBVO_M-naLVE197Ns4q7', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0").." - Kursus", content = dmessage}), { ['Content-Type'] = 'application/json' })
                        else
                            TriggerClientEvent("pNotify:SendNotification", player,{text = {nuser_id.." er allerede i det højeste level!"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        end
                    else
                        TriggerClientEvent("pNotify:SendNotification", player,{text = {"Dette ID ser ud til ikke at eksistere"}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                    end
        ...
        if vRP.hasPermission(user_id,"repair.menu") then
            choices["Mekaniker"] = {function(player,choice)
                vRP.buildMenu("mech", {player = player}, function(menu)
                    ...
                    if vRP.hasPermission(user_id, "vehicle.repair") then
                        menu["Vask køretøj"] = {ch_vaskBil, "Vask det nærmeste køretøj"}
                    end
...
AddEventHandler('onResourceStart', function(name)
    if name:lower() == GetCurrentResourceName():lower() then
      MySQL.ready(function ()
          MySQL.Async.fetchAll("SELECT * FROM bilforhandler", {}, function(rows, affected)
              if #rows > 0 then
                  local newarray = {}
                  for k,v in pairs(rows) do
                      newarray[v.vehicle] = {price=v.price,type=v.type,name=v.vehicle_name,stock=v.stock}
                  end
                  carprices = newarray
              end
          end)
