...
RegisterNUICallback(
  ...
  function(data)
    ...
    TriggerServerEvent("kasperr_inventory:dropItem", data)
      ExecuteCommand("me Smider "..data.name.."på jorden")
  end
)
...
RegisterNUICallback(
  ...
  function(data)
    ...
    TriggerServerEvent("kasperr_inventory:giveItem", data)
      ExecuteCommand("me giver "..data.name)
  end
)
...
Citizen.CreateThread(
  function()
    while true do
      Citizen.Wait(0)
      if IsControlPressed(0, Config.OpenMenu) and not vRP.isHandcuffed() and not vRP.isInComa() then
        TriggerEvent("kasperr_inventory:openGui")
      end
    end
  end
