      ...
      menudata.onclose = function(player)
		...
		vRP.getUData({user_id,"vRP:head:overlay",function(value2)
          ...
          if vRP.tryFullPayment({user_id,price}) then
            if price > 0 then
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Betalte: "..price.." DKK", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            end
          else
...
local function build_client_barbershops(source)
  ...
  if user_id ~= nil then
    for k,v in pairs(barbershops) do
      local parts,x,y,z,hidden = table.unpack(v)
...
RegisterCommand('fixhair', function(source)
  local user_id = vRP.getUserId({source})
  vRP.getUData({user_id,"vRP:head:overlay",function(value)
  if value ~= nil then
    custom = json.decode(value)
