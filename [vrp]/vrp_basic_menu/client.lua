...
function vRPbm.loadFreeze(notify,god,ghost)
	if not frozen then
		if notify then
			TriggerEvent("pNotify:SendNotification", {text = "Du er blevet frosset!", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		end
		frozen = true
		invincible = god
		invisible = ghost
		...
		if notify then
			TriggerEvent("pNotify:SendNotification", {text = "Du sat fri!", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		end
		unfrozen = true
		invincible = false
		invisible = false
	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(vehicleHandle) then
			if GetVehicleDoorsLockedForPlayer(vehicleHandle,PlayerId()) or any then
				...
				TriggerEvent("pNotify:SendNotification", {text = "Køretøj låst op!", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
				...
				TriggerEvent("pNotify:SendNotification", {text = "Køretøj allerede åbnet!", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
			end
		else
			TriggerEvent("pNotify:SendNotification", {text = "For langt væk fra Køretøjet!", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		end
	end)
end
...
function vRPbm.spawnVehicle(model)
	...
	while not HasModelLoaded(mhash) and i < 1000 do
		if math.fmod(i,100) == 0 then
			TriggerEvent("pNotify:SendNotification", {text = "Indlæser køretøj!", type = "info", queue = "global", timeout = 1000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		end
		RequestModel(mhash)
		Citizen.Wait(30)
		i = i + 1
	...
	if HasModelLoaded(mhash) then
		...
		TriggerEvent("pNotify:SendNotification", {text = "Køretøj spawnet", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
	else
		TriggerEvent("pNotify:SendNotification", {text = "Forkert model", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
	end
end

--function vRPbm.DespawnVehicle()
...
function vRPbm.deleteVehicleInFrontOrInside(offset)
	...
	if IsEntityAVehicle(veh) then
		...
		TriggerEvent("pNotify:SendNotification", {text = "Køretøj slettet!", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
	else
		TriggerEvent("pNotify:SendNotification", {text = "For langt væk fra et køretøj!", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
	end
end
