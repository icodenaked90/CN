...
function vRPgymC.populateGymTable(theWorkouts, gym, prisongym)
	gymTable = theWorkouts
	theGym = gym
	thePrisonGym = prisongym
end
...
Citizen.CreateThread(function()
	while true do
		...
		if(theGym ~= nil)then
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, theGym[1], theGym[2], theGym[3]) < 10.0)then
				...
				if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, theGym[1], theGym[2], theGym[3]) < 1.0)then
					incircle = true
					if(incircle == true) then
						gym_DisplayHelpText("Tryk ~INPUT_CONTEXT~ for at få adgang til ~g~Gym Shop")
						if(IsControlJustReleased(1, 51))then
							inGymCircle = true
		...
		if(thePrisonGym ~= nil)then
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, thePrisonGym[1], thePrisonGym[2], thePrisonGym[3]) < 10.0)then
				DrawText3D(thePrisonGym[1], thePrisonGym[2], thePrisonGym[3]+0.4, "~y~Gym Shop", 1.2)
				DrawMarker(21, thePrisonGym[1], thePrisonGym[2], thePrisonGym[3], 0, 0, 0, 0, 0, 0, 0.301, 0.301, 0.3001, 255, 255, 0, 255, 0, 0, 0, 1)
				if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, thePrisonGym[1], thePrisonGym[2], thePrisonGym[3]) < 1.0)then
					incircle = true
					if(incircle == true) then
						gym_DisplayHelpText("Tryk ~INPUT_CONTEXT~ for at få adgang til ~g~Gym Shop")
						if(IsControlJustReleased(1, 51))then
				...
				elseif(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, thePrisonGym[1], thePrisonGym[2], thePrisonGym[3]) > 1.0)then
					incircle = false
				end
			end
		end
			...
			for i, v in pairs(gymTable) do
				...
				if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 18.0)then
					...
					if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 0.25)then
						...
						if(incircle == true) then
							if(inWorkout == false)then
								gym_DisplayHelpText("Tryk ~INPUT_CONTEXT~ for at ~g~"..v[4])
								if(IsControlJustReleased(1, 51))then
									vRPserver.hasMembership({}, function(membership)
										if(membership)then
											vRPserver.initWorkout({i})
										else
											vRP.notify({"~w~[GYM] ~r~Du har ikke et gym medlemskab!"})
										end
									end)
								end
							end
					...
					if(inWorkout == true)then
						gym_DisplayHelpText("Tryk ~INPUT_CONTEXT_SECONDARY~ for at stoppe øvelsen")
						if(IsControlJustReleased(1, 44))then
							ClearPedTasksImmediately(GetPlayerPed(-1))
							inWorkout = false
							workDone = 0
