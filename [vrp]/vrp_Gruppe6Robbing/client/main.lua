      ...
      local plyPos = GetEntityCoords(player)
        ...
        if GetVehicleDoorAngleRatio(thisTruck,2) >= 0.0100 and GetVehicleDoorAngleRatio(thisTruck,3) >= 0.0100 and (not door) then
           ...
           if NetworkIsHost() then
              ...
              TriggerServerEvent('cn_notify:notifyPolice', "Der er en som røver en pengetransport her:",{robPos.x,robPos.y,robPos.z})
...
RegisterNetEvent("IsSpawned")
AddEventHandler("IsSpawned", function()
    spawn = true
end)

RegisterNetEvent("IsDeSpawned")
AddEventHandler("IsDeSpawned", function()
    despawn()
end)
