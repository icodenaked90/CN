...
AddEventHandler("Spawn", function()
    local cops = vRP.getUsersByPermission({"police.mission"})
    local antalbetjente = #cops
    if antalbetjente > 0 then
        TriggerClientEvent("IsSpawned",-1)
    end
end)

RegisterServerEvent("Despawn")
AddEventHandler("Despawn", function()
    TriggerClientEvent("IsDeSpawned", -1)
end)