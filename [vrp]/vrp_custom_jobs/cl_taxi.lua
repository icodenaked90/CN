...
function StartJob(jobid)
	if jobid == 1 then -- taxi
		...
		Wait(2000)
		DrawMissionText("Kør rundt og find ~h~~y~kunder~w~.", 10000)
		onJob = jobid
	end
end
...
function IsInTaxi()
	...
	if model == GetHashKey("TAXI_DGC2008") or model == GetHashKey("TAXI_FT2016") or model == GetHashKey("TAXI_TP2015") then
	...
	while true do
		...
		if onJob == 0 then
			if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
				if IsInTaxi() then
					DrawMissionText('Tryk E for at starte arbejdet', 10000)
					if IsControlJustPressed(1, 38) or IsDisabledControlJustPressed(1, 38) then -- E
						StartJob(1)
					end
				end
			end
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							DrawMissionText("Din kunde er  ~r~død~w~. Find en anden.", 5000)
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("Kunden ~r~er træt af at vente~w~. Find en anden.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
										if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.peds[1]), true) < 10.0001 then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("~r~Kunden vil ikke køre med dig ~w~. Find en anden.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("~c~Kør mig til %s, tæt på %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
											DrawMissionText(streetname, 5000)
										else
											local streetname = string.format("~c~Kør mig til %s", GetStreetNameFromHashKey(street[1]))
											DrawMissionText(streetname, 5000)
										end
							...
							disttom = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]]) -- I added this
							...
							if jobs.flag[1] == 3 then
								if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]], true) > 4.0001 then
									...
									TriggerServerEvent('taxi:payment', tonumber(disttom)) -- I added this
									...
									jobs.peds[1] = nil
									Wait(6000)
									DrawMissionText("~g~Du har leveret kunden!", 5000)
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									DrawMissionText("~g~Kunden~w~ venter på dig. Kør op til ham", 5000)
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
						...
						DrawMissionText("Gå tilbage til din taxa for at forsætte. Eller gå væk for at stoppe arbejdet.", 1)
					end
				end
			else
				StopJob(1)
				DrawMissionText("Taxaen er ~h~~r~smadret~w~.", 5000)
			end
		end
	end
end)