        ...
        if tattoo then
            ...
            if tattoo == "CLEAR" then -- get player weapons to not rebuy the body
                ...
                if user_id ~= nil and vRP.tryFullPayment({user_id,price}) then
                    ...
                    TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.paid({price})}, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                else
                ...
                vRP.getUData({user_id,"vRP:tattoos",function(value)
                    ...
                    if not owned then
                        ...
                        TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har allerede denne tattoo", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
                    end
                end})
            end
        end
