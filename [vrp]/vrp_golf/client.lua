...
GFServer = Tunnel.getInterface("vrp_golf","vrp_golf")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		DrawMarker(27,-1332.7823486328,128.18229675293,56.052, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 10.3, 0, 519, 0, 105, 0, 0, 2, 0, 0, 0, 0)
		--end
        ...
        if distance < 1.5 then
            ...
            if golfplaying then
                DisplayHelpText("Tryk ~g~ ~INPUT_CONTEXT~ ~s~ for at afslutte spillet.")
            else
                DisplayHelpText("Tryk ~g~ ~INPUT_CONTEXT~ ~s~ for at spille golf koster:  ~g~1000 DKK")
            end
            if (IsControlJustReleased(1, 38)) then
                ...
                if (golfplaying) then
                    ...
                    GFServer.payGolf({}, function(paid)
                      if paid then

                        spawnCart()
                        startGolf() -- If you plan to have it cost money, you need to remove this and only call it when they paid
												TriggerEvent("pNotify:SendNotification", {
												text = " - Tryk på E for at genindlæse bolden.<br/> - A og D for at rotere bolden.<br/> - Y for at ændre skifte jernet.",
												type = "success",
												timeout = 5000,
												layout = "centerRight",
												queue = "right"
                        ...
                        TriggerEvent('chatMessage', "SERVER", {255, 0, 0}, "Du har ikke nok penge.")
                      end
                    end)
                end
            end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Tryk ~g~E~s~ for at samle bolden op, hvis den sidder fast.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if golfstrokes > 12 then
		TriggerEvent("pNotify:SendNotification", {
		text = "Du brugte for mange forsøg.",
		type = "error",
		timeout = 5000,
		layout = "centerRight",
		queue = "right"
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Tryk på ~g~E~s~ for at slippe her.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Tryk ~g~E~s~ for at smide bolden - ~r~ Du er for tæt på hullet!")
		end
	end
end
...
function addBallBlip()
	...
	AddTextComponentString(tostring("Bolden"))
...
function BlipsStartEnd()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Starten af hullet"))
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Slutningen af hullet"))
...
function addblipGC()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Golf Klubben"))
...
AddEventHandler('customNotification', function(response)
	TriggerEvent('chatMessage', 'GOLF ', { 210, 235, 0 }, response)
end)
